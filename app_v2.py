...
def get_zoom_level(distance_km):
    if distance_km < 1:
        ...
        return 2  # très grande distance : zoom mondial
...
with col2:
    ...
    map_data = st_folium(m, height=390, width=800, key="map")
    ...
    # Résultat affiché sous la carte
    if st.session_state.fare is not None:
        col_fare, _ = st.columns([1, 3])  # rétrécit la zone d'affichage
        with col_fare:
